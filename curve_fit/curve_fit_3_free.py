...
def get_data(t0, M0, v0, delta_t):
    abs_t = np.abs(t0 - delta_t)          # assume t0 is from -t_E to t_E, the curve is symetrical.
...
def ln_prior(arg):
    if M[0] < arg[0] < M[-1] and v[0] < arg[1] < v[-1] and t_range[0] < arg[2] < t_range[-1]:
        return 0
    return -np.inf
...
start_point = np.c_[
    ...
    np.random.uniform(t_range[0]+1e-5, t_range[-1]-1e-5, Nwalkers)]

pool = Pool(Npro)

backend = emcee.backends.HDFBackend('backend_3p.h5')
backend.reset(Nwalkers, Ndim)
sampler = emcee.EnsembleSampler(Nwalkers, Ndim, ln_like, pool, backend=backend)
