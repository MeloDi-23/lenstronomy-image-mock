...
import math
"""
Modified from lenstronomy.LensModel.Microlening.adaptive_boundary_mesh
...
def within_distance(x, y, test_point, threshold):
...
def adaptive_boundary_mesh(
    ...
    to the source plane, and then checking if they are within the radius :math:`\\beta_s`.
     ...
     :type eta: float
     :param lensModel: The lensModel you are using
     :type lensModel: lenstronomy.LensModel.lens_model.LensModel
     :param search_window: the center of search window
     :type search_window: tuple[float, float]
    ...
    if eta is None:
        eta = 0.7*n_p
    if number_of_iterations is None or final_eta is None:
    ...
    if verbose:
        print('There will be {} loops'.format(number_of_iterations))

    # Initialize variables
    total_number_of_rays_shot = 0  # Counter for total number of rays shot
    i = 1  # Iteration counter (starts at 2 because we already have the source position)
    image_centers_x = [search_window[0]]
    image_centers_y = [search_window[1]]
    ...
    while i <= number_of_iterations:
        ...
        image_centers_x, image_centers_y, side_length = splitting_centers(
            image_centers_x, image_centers_y, side_length, n_p
        )

        # Ray shoot from image to source plane using array-based approach
        source_centers_x, source_centers_y = lensModel.ray_shooting(
            image_centers_x, image_centers_y, kwargs=kwargs_lens
        )

        if verbose:
            print(f'select all points within radius {delta_beta}')
        within_radius = within_distance(
            source_centers_x, source_centers_y, source_position, delta_beta
        )
        image_centers_x = image_centers_x[within_radius]
        image_centers_y = image_centers_y[within_radius]
        if verbose:
            print('get total {} image centres'.format(len(image_centers_x)))
        # Update total number of rays shot
        total_number_of_rays_shot += len(image_centers_x)
...
def amplification(n_pix, pixel_size, beta_s):
    return n_pix*(pixel_size**2)/(np.pi*beta_s**2)